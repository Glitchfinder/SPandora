...
public class PandoraGenerator extends ChunkGenerator
{
	...
	public Location getNearestEdge(World world, int x, int z, int period, int count,
		boolean inner, PandoraBiome target)
	{
		if (world == null)
			return null;
		...
		if (cEdge == null)
			...
			return getNearestEdge(world, x, z, 1, diff, true, target);
		}
		...
		boolean inner)
	{
		...
		Location center = new Location(world, x, 0D, z);
		Map<Location, PandoraBiome> biomes = new HashMap<Location, PandoraBiome>();
		biomes.put(center, biome);

		for (int cx = (x - edgeMax); cx <= xMax; cx += period) {
			for (int cz = (z - edgeMax); cz <= zMax; cz += period) {
				PandoraBiome cGen = getGenerator(world, cx, cz);

				if (biomes.values().contains(cGen))
					continue;

				biomes.put(new Location(world, cx, 0D, cz), cGen);
			}
		}

		if (biomes.keySet().size() <= 1)
			return null;

		Location edge = center;
		PandoraBiome edgeBiome = biome;

		for (Location loc : biomes.keySet()) {
			PandoraBiome cGen = biomes.get(loc);
			if (cGen.priority < edgeBiome.priority)
				continue;
			else if (cGen.priority != edgeBiome.priority && generators.indexOf(cGen) >= generators.indexOf(edgeBiome))
				continue;
		...
		if (edge == center)
			return null;//(inner ? edge : null);
/*
		if (!inner) {
			int xDiff = Math.abs(x - edge.getBlockX());
			int zDiff = Math.abs(z - edge.getBlockZ());
			int diff = ((xDiff > zDiff) ? xDiff : zDiff);

			return getNearestEdge(world, x, z, 1, diff, true);
		}
*/
		return getNearestEdge(world, x, z, period, count, false, edgeBiome);
	}

	public PandoraEdge getNearestEdgeBiome(World world, int x, int z, int period, int count,
		boolean inner)
	{
		if (world == null)
			return null;
...
*/
		PandoraEdge result = new PandoraEdge();
		result.biome = edgeBiome;
		result.location = getNearestEdge(world, x, z, period, count, false, edgeBiome);
		return result;
	}
	...
	public class PandoraEdge {
		public PandoraBiome biome;
